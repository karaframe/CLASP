#' Function to transform an irregular time-series to a continuous time-series. 
#' 
#' \code{pad_multiday} is usually used when irregularly sampled variables are to
#' be analysed alongside variables which are monitored at regular hourly or 
#' daily intervals. 
#'
#' \code{pad_multiday} will pad a variable's time-series and then push values 
#' forwards to replace missing data after padding. This is known as a "last 
#' observation carried forward" (locf) process. 
#' 
#' It is not recommended that the transformed variables are used for 
#' aggregations. \code{pad_multiday} will introduce values into a time-series 
#' and if these are aggregated, in most cases, this will be a flawed procedure.
#' 
#' \code{pad_multiday} requires a data frame with a \code{"date"} and 
...
#' be \code{"day"} or \code{"hour"} but also accepts \code{"sec"} and 
#' \code{"min"}.
#' 
#' @param by Should \code{pad_multiday} apply the padding function to groups 
#' within \code{df}? This is helpful when there are many sites/other identifiers
#' within \code{df} which need to be padded individually. 
#' 
...
#' 
#' @seealso \code{\link{na.locf}}, \code{\link{round_date}}, 
#' \code{\link{import_measures}}
#' 
#' @import dplyr
#' 
...
#' @examples 
#' \dontrun{
#' # Query database for a multi-day pollutant
#' data_benzene <- import_measures("archive", "yk11", "benzene", start = 1970)
#' 
#' # Make a continuous time-series
#' data_benzene_continuous <- pad_multiday(data_benzene, interval = "day")
#' 
#' 
#' # Many sites can be done at once too
#' data_two <- import_measures("archive", c("yk11", "leed"), "benzene", 
#'                              start = 1970)
#' 
#' # Pad                                
#' data_two_continuous <- pad_multiday(data_two, interval = "day", by = "site")
#' 
#' # Plot
#' timePlot(data_two_continuous, "benzene", type = "site")
#' 
#' }
#' 
#' @export
pad_multiday <- function(df, interval, by = NA, na.rm = FALSE) {
  ...
  # Check by
  if (length(by) > 1) stop("'by' can only take a single value.", call. = FALSE)
...
irregular_padder <- function(df, interval, by = NA, na.rm) {
  ...
  df <- dplyr::add_rownames(df, "row_number")
  df$row_number <- as.integer(df$row_number)
  ...
  if (ncol(df) == 2) {
    ...
    df[, -1] <- lapply(df[, -1], function(x) zoo::na.locf(x, na.rm = FALSE))
  ...
  if (!is.na(by)) {
    ...
    # Make a character to avoid a dplyr binding warning
...
# A simple time padder
padder <- function(df, interval) {
